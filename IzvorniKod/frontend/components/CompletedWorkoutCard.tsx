...
interface CompletedWorkoutsCardProps {
  workout: {
    ...
    exercises: Array<{
      ...
      name: string;
      sets: Array<{
        // Menjamo tip "sets" kako bi bio niz
        reps: number;
        RP: number;
      }>;
    }>;
...
const CompletedWorkoutsCard: React.FC<CompletedWorkoutsCardProps> = ({
  workout,
}) => {
  return (
      ...
      <ul>
        {workout.exercises.map((exercise) => (
          <li key={exercise.id} className="mb-2">
            <p className="text-xs font-medium text-gray-800">{exercise.name}</p>
            <ul className="pl-3">
