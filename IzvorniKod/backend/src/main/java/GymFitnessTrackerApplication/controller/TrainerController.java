...
public class TrainerController {
    ...
    public ResponseEntity<?> trainUser(@RequestHeader("Authorization") String token,@PathVariable String id){
        ...
        MyUser trainer = (MyUser) myUserService.getMyUser(email);
        if(!trainer.getRole().equals(Role.TRAINER)) throw new AdminRestrictedException("User not trainer");
        
        MyUser client = myUserService.getMyUserByID(id);
        String jwtToken = jwtService.generateTokenForClient(trainer, client);
        
        return ResponseEntity.status(200).body(new JwtResponse(jwtToken, trainer.getId().toString(), trainer.getName(), trainer.getEmail(), trainer.getRole().toString()));
    }
}
