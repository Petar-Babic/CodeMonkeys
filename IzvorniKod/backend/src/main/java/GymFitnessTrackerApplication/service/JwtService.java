...
public class JwtService {
    ...
    public Claims getJwt(String token){
        return Jwts.parser()
    ...
    public String generateTokenForClient(MyUser trainer, MyUser client) {
        return Jwts.builder()
                .subject(client.getEmail())
                .claim("role", trainer.getRole().toString())
                .claim("trainerEmail", trainer.getEmail())
                .issuedAt(Date.from(Instant.now()))
                .expiration(Date.from(Instant.now().plusMillis(VALIDITY)))
                .signWith(generateKey())
                .compact();
